 ...
 * gutter 多个columns 时， gutter 为行之间的间距
 * element
 *  id | key 对应表单字段名 isRequired
 *  label 显示的 label
 *  tooltip label 右侧显示 tooltip
 *  widget 表单显示的部件
 *
 * fieldProps - getFieldDecorator(id, options) 对应的为 options (可以写整个对象也可以写其中的属性。 FormBuilder 已经处理好了)
 *      initialValue 初始值
 *      rules  部件的校验规则
 *      ...
 ...
 */
const formMeta = {
  colon: true, // 配合 label 属性使用，表示时否显示 label 后面的冒号
